...
harness does NOT incorporate or redistribute any of myPKA's source text. The prose, identity model (TOWER + 12 callsigns), folder layout (`PKM/`, `Team/`, `Team Knowledge/`, `Team Inbox/`, `Deliverables/` matching myPKA; additional `state/`, `adapters/`, `bootstrap/`, `ci/`, and `repos/` trees diverging for harness's infra-management nature), and bootstrap procedure are authored fresh for harness and released under MIT. Files that mirror myPKA's structure (notably `ADAPTER-PROMPT.md` and the root `AGENTS.md`) include an inline attribution note at the bottom.
