...
- Verdict: move-to-harness as team-memory under `Principal/Reference/` or a `Team Knowledge/Guidelines/`
...
- Verdict: move-to-harness as `Principal/Reference/<topic>.md` (per Phase-0-audit §2 mapping).
...
- Verdict: drop (regenerate). Phase-0-audit says DROP — replaced by thin pointer generated by
  ADAPTER-PROMPT. The environment facts belong in `Principal/.user.yaml` or a reference file;
